...
// Mock DB and validator before requiring the route module
...
const serviceRoutes = require('./people.routes');

const app = express();
app.use(express.json());
app.use('/', serviceRoutes);

// Error handler to expose Boom errors as HTTP responses during tests
app.use((err, req, res, next) => {
  if (err && err.isBoom) return res.status(err.output.statusCode).json(err.output.payload);
  res.status(500).json({ message: err.message || 'Internal Server Error' });
});

// Configure DB mock behavior for queries used by the routes
